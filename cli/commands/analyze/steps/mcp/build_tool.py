...
from cli.commands.analyze.steps.mcp.identify import format_request_details
...
class BuildToolStep(Step[ToolBuildInput, ToolBuildResult]):
    ...
    async def _execute(self, input: ToolBuildInput) -> ToolBuildResult:
...
{target_details}"""
        ...
        return ToolBuildResult(tool=result.tool, consumed_trace_ids=result.consumed_trace_ids)

    def _validate_output(self, output: ToolBuildResult) -> None:
        tool = output.tool
